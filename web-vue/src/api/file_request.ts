...
service.interceptors.request.use(
  (config) => {
    // 从localStorage获取token (与userStore保持一致)
...
const fileRequest = {
   ...
   * @param username 用户名，用于区分文件归属
   * @param config 额外的请求配置
   * @returns { url: string, bucket: string, objectKey: string }
   */
  upload<T = { url: string, bucket: string, objectKey: string }>(bucket: string, file: File, isCache?: boolean, username?: string, config?: AxiosRequestConfig): Promise<T> {
