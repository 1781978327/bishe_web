...
inline std::string resolve_project_root_path() {
    ...
    for (const auto& seed : seeds) {
        ...
        for (int depth = 0; depth < 6 && !cur.empty(); ++depth) {
            if (looks_like_project_root_path(cur)) {
                cached_root = cur;
                return cached_root;
            }
...
inline std::string resolve_reid_model_path_locked() {
    ...
    std::string project_candidate = resolve_project_file_path("model/RK3588/osnet_x0_25_market.rknn");
    if (!project_candidate.empty()) candidates.push_back(project_candidate);
    candidates.push_back("../model/RK3588/osnet_x0_25_market.rknn");
    candidates.push_back("./model/RK3588/osnet_x0_25_market.rknn");
...
inline void load_labels(const char* model_path) {
    ...
    // 默认优先使用 coco 标签 txt
    std::string project_label_path = resolve_project_file_path("model/coco_80_labels_list.txt");
    if ((!project_label_path.empty() && load_labels_from_txt(project_label_path)) ||
