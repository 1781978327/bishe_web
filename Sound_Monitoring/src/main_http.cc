 ...
 *
 *   POST /realtime/start     - 启动实时麦克风监测 (USB摄像头麦克风 card 4)
 *                             Body: {"device": "plughw:4,0"} (可选，默认 plughw:4,0)
...
static char rt_device[64] = "plughw:4,0";
...
static void *rt_monitor_thread(void *arg) {
    ...
    int frames_per_chunk = rt_sample_rate * RT_CHUNK_DURATION_SEC;
    int frames_per_hop = rt_sample_rate * (int)RT_HOP_SEC;
    ...
    while (rt_running) {
        ...
        if (got > 0) {
            rt_ring_push(&rt_ring, capture_get_data(rt_cap), got);
        }
...
static int analyze_audio(const char *audio_path, char *result_json, size_t json_size) {
    ...
    written = snprintf(json_ptr, json_size,
        ...
        "\"total_chunks\": %d, \"anomaly_count\": %d, \"anomaly_saved\": %s, \"events\": [",
...
void handle_realtime_start(int client_fd, const char *body) {
    ...
    if (ret == 0) {
        snprintf(result, sizeof(result),
            "{\"success\": true, \"message\": \"Realtime monitoring started\", \"device\": \"%s\"}", device);
        send_response(client_fd, "200 OK", "application/json", result, strlen(result));
    } else if (ret == -1) {
        snprintf(result, sizeof(result),
...
void handle_config(int client_fd) {
    ...
    snprintf(body, sizeof(body),
        "{\"model_path\": \"%s\", \"anomaly_threshold\": %.2f, \"save_anomaly\": %d, \"keywords_count\": %d}",
...
void handle_client(int client_fd) {
    ...
    while (total_len < (int)sizeof(buffer) - 1) {
        int len = recv(client_fd, buffer + total_len, sizeof(buffer) - 1 - total_len, 0);
        if (len <= 0) {
            close(client_fd);
            return;
        }
        ...
        if (header_len < 0) {
            ...
            if (header_end) {
                ...
                if (header_len + content_length >= (int)sizeof(buffer)) {
                    send_json_response(client_fd, 1, "Request body too large");
                    close(client_fd);
                    return;
                }
            }
    ...
    const char *ct_line = strstr(buffer, "Content-Type:");
    if (ct_line && ct_line < buffer + header_len) {
        const char *ct_end = strstr(ct_line, "\r\n");
        if (ct_end && ct_end < buffer + header_len) {
